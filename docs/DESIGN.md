...
The former Explore frames are retained only as historical design references. The
current `/search` route is intentionally a lightweight utility with one
URL-backed query field and grouped canonical Drink and Store results; it does
not reproduce the former editorial surface.
